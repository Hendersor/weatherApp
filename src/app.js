...
const getDate = () => {
  ...
  const finalDate = `${month}/${dayMonth}/${year}`;
...
async function getWeather() {
  const response = await fetch(
    `http://api.openweathermap.org/geo/1.0/direct?q=${cityName}&appid=86e024a5e8c23b961035736b9f56fd33`
  );
  ...
  const weatherFetch = await fetch(
    `https://api.openweathermap.org/data/2.5/onecall?lat=${cityLat}&lon=${cityLon}&units=metric&exclude={part}&appid=86e024a5e8c23b961035736b9f56fd33`
  );
  ...
  const dayClasses = document.querySelectorAll(
    ".daysWeatherContainer .day .week"
  );
  const iconWeekClasses = document.querySelectorAll(
    ".daysWeatherContainer .day .weekIconContainer"
  );
  ...
  for (let i = 0; i < iconWeekClasses.length; i++) {
    ...
    iconWeekClasses[
      i
    ].src = `http://openweathermap.org/img/wn/${weatherIconWeek}@4x.png`;
